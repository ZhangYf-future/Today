...
class WeatherInfoPresenter
    extends BasePresenter<WeatherInfoModel, WeatherInfoState> {
  //上一次请求的城市信息
  String? _oldCityInfo;
  ...
  void getWeatherNowInfo(String cityInfo) async {
    ...
    if (cityInfo == _oldCityInfo &&
        !DateUtils.checkMoreThan20Minutes(_oldRequestTime)) {
      //直接退出，不会发起请求
  ...
  void _getWeatherHourInfo(String cityInfo) async {
    final WeatherHourBean? result =
        await this.model.getWeatherHourInfo(cityInfo);
  ...
  void _getWeatherSevenDay(String cityInfo) async {
    final WeatherSevenDayBean? result =
        await this.model.getWeatherSevenDay(cityInfo);
  ...
  void _getWeatherLifeInfo(String cityInfo) async {
    final WeatherLifeBean? result =
        await this.model.getWeatherLifeInfo(cityInfo);
    if (checkHttpResult(result, result?.daily)) {
      //数据请求成功
  ...
  void _getWeatherWarningInfo(String cityInfo) async {
    final WeatherWarningBean? result =
        await this.model.getWeatherWarningInfo(cityInfo);
  ...
  void _getAirQualityInfo(String cityInfo) async {
    final WeatherAirQualityBean? airQualityResult =
        await this.model.getWeatherAirQualityInfo(cityInfo);
...
class WeatherInfoModel extends BaseModel<WeatherInfoPresenter> {
  ...
  //请求逐小时天气信息
  Future<WeatherHourBean?> getWeatherHourInfo(String cityInfo) =>
      weatherHelper.getWeatherHour(cityInfo);

  //请求未来七天天气预报
  Future<WeatherSevenDayBean?> getWeatherSevenDay(String cityInfo) =>
      weatherHelper.getWeatherSevenDay(cityInfo);

  //请求当天的生活指数
  Future<WeatherLifeBean?> getWeatherLifeInfo(String cityInfo) =>
      weatherHelper.getWeatherLifeInfo(cityInfo);

  //请求当天的报警信息
  Future<WeatherWarningBean?> getWeatherWarningInfo(String cityInfo) =>
      weatherHelper.getWeatherWarningInfo(cityInfo);

  //请求空气质量信息
  Future<WeatherAirQualityBean?> getWeatherAirQualityInfo(String cityInfo) =>
      weatherHelper.getWeatherAirQualityInfo(cityInfo);
}
